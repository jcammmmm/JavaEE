 ...
 * CountryDb entities. Transaction control of the save(), update() and delete()
...
public class CountryDbDAO extends BaseHibernateDAO {
	...
	public void save(CountryDb transientInstance) {
		...
		System.out.println("saving CountryDb instance");
	...
	public CountryDb findById(java.lang.Integer id) {
		...
		try {
			CountryDb instance = (CountryDb) getSession().get("CountryDb", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(CountryDb instance) {
		...
		try {
			List results = getSession().createCriteria("CountryDb").add(Example.create(instance)).list();
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding CountryDb instance with property: " + propertyName + ", value: " + value);
		try {
			String queryString = "from CountryDb as model where model." + propertyName + "= ?";
	...
	public CountryDb merge(CountryDb detachedInstance) {
		...
		try {
			CountryDb result = (CountryDb) getSession().merge(detachedInstance);
